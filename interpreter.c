    ...
    while(i<n)
    {
        ...
        switch(exec(line))
        {
            ...
            case TO_EVAL:
                ...
                new = eval(i, line, code[i], all_vars);
                if(new==i)
                    free(code[i]);
            ...
            case CHANGE:
                //Change line code
                printf("%s", LINE_CHANGE_MSG);
                scanf("%d", &line_no);
                if(line_no>=i|| line_no<0)
                    printf("%s\n", INVALID_LINE_MSG);
                printf("current line number %d is\n %s\n", line_no, line);
                eval(line_no, line, code[line_no], all_vars);
                printf("%s\n", NEW_LINE_INPUT_MSG);
                scanf("%s", line);
...
int execute_code(prog_vars_vals* init_var_vals, all_macros* macro)
{
    ...
    bool check_fin;
    prog_vars_vals* curr_var_vals = init_var_vals;
